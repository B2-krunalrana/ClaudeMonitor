...
// Base class for notification cards with buttons (LinkedIn, QoL)
class ButtonNotificationCard extends FloatingCard {
	...
	addKofiButton() {
		...
		link.className = 'ut-block ut-text-center';
		link.style.marginTop = '10px';
		link.style.cssText += '; display: inline-block; background: #0a66c2; color: white; padding: 6px 12px; border-radius: 6px; font-size: 12px;';
		link.textContent = '❤️ Made with love by Krunal Rana';
		this.element.appendChild(link);
	}
	...
	async addDesktopFooter() {
		...
		link.textContent = '💼 Connect with Krunal Rana';
...
class VersionNotificationCard extends ButtonNotificationCard {
	...
	build() {
		...
		patchNotesLink.href = 'https://www.linkedin.com/in/krunal-rana/';
